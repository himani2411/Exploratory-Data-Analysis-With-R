...
```{r}
theme_g = theme(panel.grid.major = element_line(color = "grey80"), panel.grid.minor = element_line(color = "grey80"),
...
ggplot(income_data_race %>% 
  ...
  xlab("Year") + 
  ylab("Median Income($/week)") + 
  ggtitle("Median income over years Vs racial groups") +theme_g
...
```
With age, the median income rises. The age group 25-34 has witnessed an increase in median income in recent years, whereas the rest of the categories have seen decreases. In the 2019-2020 year, the age groups 18-24 and 54 showed a considerable reduction, implying that most young and older employees lost more jobs than other age groups during the epidemic. 
...
ggplot(temp, aes(x = YEAR, y = Average_median_income)) +
  ...
  # facet_wrap(~SEX + RACE_GRP, ncol = 5, labeller = label_context) +
   facet_wrap(~SEX + RACE_GRP, ncol = 5)+
   theme_g
```
...
ggplot(income.lm, aes(x = YEAR, y = MEDIAN_INCOME)) + 
  ...
  facet_wrap(~GENDER + RACE_GRP, ncol = 5) +
...
ggplot(income.lm.df, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  #geom_smooth(method = "loess", color = "orange", method.args = list(degree = 1)) +
  ylim(-2, 2) +
  theme_g +
...
ggplot(income.race_MajorIND.pred.df, aes(x = YEAR, y = pred)) +
  ...
  # facet_wrap(~RACE_GRP + MAJOR_IND, labeller = label_both) + 
  facet_wrap(~RACE_GRP + MAJOR_IND)+
  # theme(panel.grid.major = element_line(color = "grey80"), panel.grid.minor = element_line(color = "grey80")) +
  theme_g+
...
ggplot(income.lm8, aes(x = YEAR, y = MEDIAN_INCOME)) + 
  geom_smooth(method = "lm", se = FALSE) +
   labs(title="Log(Median Income) of each race group by gender group over the years", 
       x="Year", 
       y="Log( Median Income )")+
  #geom_smooth(method = "loess", se = FALSE, color = "orange", method.args = list(degree = 1)) +
  facet_wrap(~GENDER + RACE_GRP, ncol = 5) +theme_g
  # theme(panel.grid.major = element_line(color = "grey80"), panel.grid.minor = element_line(color = "grey80"),
  #       text = element_text(size = 12), plot.title = element_text(hjust = 0.5))
```
...
ggplot(income.lm8.pred.df, aes(x = YEAR, y = expm1(income.prob), group = MAJOR_IND, color = MAJOR_IND)) +
  ...
  facet_grid(EDUC_GRP + AGE_GRP ~ RACE_GRP + GENDER)+
  theme(panel.grid.major = element_line(color = "grey80"), panel.grid.minor = element_line(color = "grey80"),
